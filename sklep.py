...
# ------------ Служебные параметры, чтобы вести статистику --------
myquelen = 0  # Текущаяя длина очереди
queue = []  # массив длин очереди, для построения графика
maxwaits = []  # массив времен ожидания в очереди, для построения графика
timelist_q = []  # массив времен, соответствующий длинам очереди queue
timelist_w = []  # массив времен, соответствующий временам ожидания
# -----------------------------------------------------------------
...
# один шаг моделирования - 1 секунда реального времени; таким образом
# процесс моделирования составляет 14 часов; 
# посетители ходят только 10 часов (см source_men)
...
plot(timelist_q, queue, label='queue_len')
title('Queue length')
xlabel(u'Simulation time, sec')
ylabel(u'Current queue length, #')
figure()
# График времени ожидания в очереди
plot(timelist_w, maxwaits)
title(u'Waiting time')
xlabel(u'Simulation time, sec')
ylabel(u'Somebody waits..., sec')
show()
# ------------------------------------------------------------------------------------------
